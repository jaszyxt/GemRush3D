...
> **Note:** `com.unity.postprocessing` is embedded under `Packages/` with two
> small Unity 6 compatibility patches. It is **not** used at runtime — its
> `Init()` needs a resource asset that can't reach a player build under this
> project's zero-imported-assets rule. Remove it (manifest line + folder) if
